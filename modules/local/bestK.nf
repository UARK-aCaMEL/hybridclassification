process BESTK {
    ...
    BEST_MATCH=\$(find "${best_results}/" -type f -name "ClumppIndFile.output.\$K" -print)
    if [ ! -f "\$BEST_MATCH" ]; then
        echo "❌ Could not find ClumppIndFile.output.\$K in ${best_results}" >&2
        exit 1
    fi
    ln -s "\$(realpath "\$BEST_MATCH")" best_clumpp_indfile.out

    # Also locate the file for K=2
    K2=2
    K2_MATCH=\$(find "${best_results}/" -type f -name "ClumppIndFile.output.\$K2" -print)
    if [ ! -f "\$K2_MATCH" ]; then
        echo "❌ Could not find ClumppIndFile.output.\$K2 in ${best_results}" >&2
        exit 1
    fi
